...
import { UnirollVue } from "../src/plugins/vue-plugin"

test("rollup-plugin-vue parsed style query", async () => {
  const transform = UnirollVue().transform as (code: string, id: string) => Promise<SourceDescription>;
...
test("rollup-plugin-vue not parsed query", async () => {
  const transform = UnirollVue().transform as (code: string, id: string) => Promise<string>;
  const id = "/index.js";
